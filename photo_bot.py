from aiogram import Bot, types, Dispatcher, Router, F
from aiogram.filters import Command
import asyncio
import uuid
import json
import websockets


token = os.getenv("TOKEN")
runware_api =  os.getenv("RUNWARE_API")

bot = Bot(token=token)
dp = Dispatcher()
router = Router()

async def create_image(prompt): 
    async with websockets.connect('wss://ws-api.runware.ai/v1') as websocket:
        auth_request = [{"taskType": "authentication","apiKey": runware_api}]
        await websocket.send(json.dumps(auth_request)) 
        auth_response = await websocket.recv()
        n = 1
        image_request = [{"positivePrompt": prompt,"model": "runware:100@1",'steps':4,'width':512,'height':512,'numberResults':n,'outputType':['URL'],'taskType':'imageInference','taskUUID':uuid.uuid4().hex}]
        await websocket.send(json.dumps(image_request)) 
        img = await websocket.recv()
        data = json.loads(img)['data'][0]
        return data

@router.message(Command('start')) 
async def start(message: types.Message):
    await message.answer('Привет!\nЯ бот созданный @rayhartt для генерации изображений.\nНапишите промт на английском языке.')

@router.message(Command('donate')) 
async def start(message: types.Message):
    monospace_text = "Бот работает на бесплатной основе, если вам он понравился, то вы можете поддержать создателя:\n\nBTC: `36zgyAAKYhDp43BoAmMk7wKf1SSJ9bMDu1`\nСеть: `BTC-Bitcoin`\n\nTON: `EQD5vcDeRhwaLgAvralVC7sJXI-fc2aNcMUXqcx-BQ-OWnOZ`\nСеть: `TON-TON`\nMemo: `10266128`\n\nETH: `0x03c59bef0d53df8ef05fcfb277ccd14d6998aaad`\nСеть: `ETH-Arbitrum One`"
    await message.reply(monospace_text, parse_mode='MarkdownV2')

@router.message() # Ловим промпты
async def gen(message: types.Message):
    if not message.text:
        return await message.answer('Отправьте мне запрос текстом на английском языке')

    
        

    msg = await message.answer(f'Начинаю генерацию по запросу:\n\n{message.text}')
    image = await create_image(message.text)
    await message.answer_photo(image['imageURL'], caption=f'Ваша генерация по запросу:\n\n{message.text}')
    await bot.delete_messages(message.chat.id, [msg.message_id, message.message_id])

async def main():
    router.message.filter(F.chat.type == 'private')
    dp.include_router(router)
    await dp.start_polling(bot)

if __name__ == '__main__':
    print('Бот для генерации картинок успешно запущен')
    asyncio.run(main())
